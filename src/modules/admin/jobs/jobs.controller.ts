...
export class JobsController {
  ...
  @ApiQuery({ name: 'timeFilter', required: false, enum: ['today', 'yesterday', 'this_week', 'this_month'], description: 'Filter by time period' })
  getAllJobs(
    @Query('page') page?: number,
    @Query('limit') limit?: number,
    @Query('status') status?: JobStatus,
    @Query('timeFilter') timeFilter?: 'today' | 'yesterday' | 'this_week' | 'this_month'
  ) {
